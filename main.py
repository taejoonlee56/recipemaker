...
async def post_streaming_url(
    ...
    try:
        ...
        if stream:
            ...
            async def stream_generator():
                ...
                async for line in r.content:
                    ...
                    if content in content_map:
                        content = content_map[content]  # Replace content using the map
                        line_json["message"]["content"] = content
                        line = (json.dumps(line_json) + "\n").encode("utf-8")
                        yield line
                        continue
                    
                    if '{' in content:
                        sg_is_json_object = True  # Start accumulating
                        sg_json_buffer = ""       # Reset buffer

                    if sg_is_json_object:
                        sg_json_buffer += content  # Accumulate content
                        print("sg_json_buffer:", sg_json_buffer)

                        line_json['message']['content'] = ""
                        line = (json.dumps(line_json)+"\n").encode('utf-8')
                        if '}' in content:
                            sg_is_json_object = False  # Stop accumulating
                            try:
                                json_response = json.loads(sg_json_buffer)
                                sg_is_json_object = False
            ...
            res = await r.json()
            print("Response JSON:", res)  # Print JSON response for non-streaming
